...
export async function subscribe(extensionId: string, listenerUri: string) {
  ...
  switch (res.status) {
    case 200:
      console.log(
        '[telemetry-api:subscribe] Subscription success:',
        await res.text()
      )
      break
    case 202:
      console.warn(
        '[telemetry-api:subscribe] Telemetry API not supported. Are you running the extension locally?'
      )
      break
    default:
      console.error(
        '[telemetry-api:subscribe] Subscription failure:',
        await res.text()
      )
      break
  }
